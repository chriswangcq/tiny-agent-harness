...
> tiny-agent-harness 是一个以 bash 为唯一外部动作面、以显式状态机和 durable artifacts 为核心的 coding agent runtime 骨架。
...
| 统一外部动作面 | 模型可见工具只有 `bash`；MCP、memory、skills、sub-agent、code intelligence、测试、git 等能力都通过 CLI 进入 bash session。 |
...
| 差异点 | 说明 |
| --- | --- |
| bash-only external action | 不把能力注册成一组 provider tools，而是把外部动作统一收敛到 bash session。 |
...
| 优势 | 说明 |
| --- | --- |
| 核心约束简单 | bash 是唯一外部动作面，降低模型工具协议和 harness 内核复杂度。 |
...
| 代码理解能力不污染内核 | LSP 能力通过 `codeq` CLI 暴露，不改变 bash-only 约束。 |
...
| 风险 | 说明 | 建议 |
| --- | --- | --- |
| typecheck 尚未通过 | TUI 当前存在 2 个 TypeScript 类型错误。 | 优先修复 TUI renderer/controller 类型契约，恢复 `npm run typecheck` 绿灯。 |
...
| bash-only 约束对模型要求高 | 模型必须学会通过 bash 调 skill/codeq/im 等 CLI，而不是直接获得 typed tool affordance。 | 在 system prompt 和 examples 中强化常见 CLI 使用路径。 |
...
1. tiny-agent-harness keeps the model-visible action surface to bash only.
...
1. tiny-agent-harness 让模型可见的外部动作面只保留 bash。
2. tiny-agent-harness 把 agent 执行过程变成显式状态和可持久化日志。
3. tiny-agent-harness 让 skill、代码智能、IM 和未来工具以 CLI 生态演进，而不是膨胀内核。

## 11. 结论

`tiny-agent-harness` 已经形成一个清晰的 coding agent runtime 骨架：DeepSeek V4 FIM two-pass model adapter、显式 run state、orchestrator effect loop、bash-only tool boundary、PTY session manager、environment event model、skill lifecycle、code intelligence CLI、state storage、transcript 和 TUI player。

按当前快照估算，项目约 1.95 万行统计源码/文档/测试，其中测试约 6 千行、文档约 5 千行。它的工程投入重点不是堆功能，而是把 agent 执行过程变得可解释、可审计、可恢复。

当前最值得继续推进的是：修复 typecheck、统一 state root 落地、打通 CLI bin 入口、强化 TUI 观察和控制能力，并把 skill 复盘 lessons 机制发展成未来 skill 进化的基础。
