...
- When `events.jsonl` is configured, `Environment` also watches the JSONL file while waiting so sibling CLI commands such as `tiny-agent skill close` can wake the run.
