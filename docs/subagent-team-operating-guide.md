...
tiny-agent im pair --runtime-host-socket <edge-socket> --a user:main --b member:team-p6/coder-1 --kind a2a
tiny-agent im bind --runtime-host-socket <edge-socket> --run-id run-123 --self member:team-p6/coder-1 --peer user:main --kind a2a
tiny-agent im post --runtime-host-socket <edge-socket> --from user:main --to member:team-p6/coder-1 --text "Fix roster projection and report commit/test evidence"
tiny-agent im post --runtime-host-socket <edge-socket> --from user:main --to member:team-p6/coder-1 --text-stdin < assignment-instructions.md
```

The external dispatch path is an edge runtime replica socket. Use the roster to discover or record the member's `runId` and endpoint metadata; public IM owns delivery. The team roster can store an optional `assignment` label for observability, but it does not mirror IM delivery state.
...
6. Send concrete instructions with `tiny-agent im post --runtime-host-socket <edge-socket> --from user:main --to member:<teamId>/<memberId> --text ...`.
