...
当前目标协议仍然是 PTY-first，但模型不再看到一个混杂的 `bash` action union。可见动作拆成 terminal input tools 和 session management tools。所有 PTY observation 统一成 bounded visual viewport：默认看最新一屏，也可以用 visual-line cursor 翻阅 retained semantic scrollback。
...
5. `session_observe` 返回 current 或指定 session 的 semantic terminal viewport，不改变 current session；`startLine` / `lineCount` 用于 visual-line paging。
6. Observation 不是 raw log 分页 API。完整 raw 输出写入 session log；需要搜索或精确 raw 历史时，agent 使用 bash 原生命令读取日志。
...
`screen.text` is a semantic terminal viewport snapshot, not a raw log tail. It should fit the requested visual-line window: by default the latest configured terminal screen, such as `rows=30`, `cols=120`; for `session_observe`, an explicit `startLine` and `lineCount` can page retained semantic scrollback. `screen.window` is the stable cursor surface for this retained semantic projection: visual lines are 0-based from the session scrollback top, and the returned range is `[startLine,endLine)`. Managed shell markers and continuation prompt chrome are stripped from this model-facing viewport; the main prompt remains visible to preserve cwd/user orientation. Raw PTY bytes remain auditable through `screen.logRef.path`. In the CLI runtime this path is a run-scoped file such as `.tiny-agent/runs/<runId>/sessions/<safe-session-id>-<sha256-10>.log`; unconfigured tests may use a virtual fallback.
...
`screen.truncated` means the current visual window is not the full retained semantic scrollback. The agent should use `session_observe` with `startLine` / `lineCount` to read nearby visual history, and should read `screen.logRef.path` with bash-native commands when it needs raw search, exact byte history, or more than bounded observation pages:
