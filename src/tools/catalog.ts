...
export const BASH_TOOL_DEFINITION: ToolDefinition = {
  ...
  description:
    ...
    "Never use shell heredocs for generated files, code, HTML, Markdown, JSON, or multiline IM replies. Heredocs are only an escape hatch for tiny fixed shell-control snippets with predictable literal content. " +
    "For generated or multiline payloads, avoid shell parsing by starting a foreground stdin consumer such as `cat > path\\n` or `node dist/cli/main.js im send --channel <channel> --kind status --text-stdin\\n`, polling until it is clearly waiting for input, writing the payload text directly, sending ctrl-d, and polling until the shell prompt returns. End text payloads with \\n before ctrl-d. If the payload does not end with \\n, one ctrl-d may only flush the current line while the foreground program keeps reading; do not send any further shell command until a prompt returns, and send a second ctrl-d if needed. " +
    "For short single-line IM replies with safe shell text, write an IM send command such as `node dist/cli/main.js im send --channel <channel> --kind status --text '<reply>'\\n`. For any reply containing newlines, quotes, backticks, pipes, or dollars, use the stdin consumer flow instead. " +
