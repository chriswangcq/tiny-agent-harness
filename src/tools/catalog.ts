...
export const BASH_TOOL_DEFINITION: ToolDefinition = {
  ...
  description:
    ...
    "Use shell heredocs sparingly: they are acceptable only for short, simple, predictable snippets. Do not use heredocs for large generated files, HTML, Markdown, JSON containing backticks/dollars/quotes, or content you are not sure the shell can parse literally. For large or complex generated text/code files, avoid shell parsing by starting a foreground stdin consumer such as `cat > path\\n`, polling until it is clearly waiting for input, writing the file text directly, then sending ctrl-d and polling until the shell prompt returns. End text payloads with \\n; if not, ctrl-d may need to be sent twice. " +
    "For short single-line IM replies, write an IM send command such as `node dist/cli/main.js im send --channel <channel> --kind status --text '<reply>'\\n`. " +
    ...
    "For small/simple generated text files or code, shell heredocs or small scripts through write_text are also fine; prefer foreground stdin consumers for complex text. The runtime paces large writes internally. " +
