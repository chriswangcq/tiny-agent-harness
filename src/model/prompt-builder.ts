...
const SYSTEM_MESSAGE =
  ...
  "  Runtime pacing solves PTY transport, not shell parsing. Use shell heredocs sparingly: they are acceptable only for short, simple, predictable snippets. Do not use heredocs for large generated files, HTML, Markdown, JSON containing backticks/dollars/quotes, or content you are not sure the shell can parse literally. To write large or complex text/code, start a foreground stdin consumer such as `cat > path\\n`, poll until the PTY appearance shows it is waiting for input, write the file text directly, then send ctrl-d and poll until the shell prompt returns. End text payloads with `\\n`; if not, ctrl-d may need to be sent twice.\n" +
  ...
  "  For short single-line IM replies, run `node dist/cli/main.js im send --channel <channel> --kind status --text '<reply>'\\n` through write_text.\n" +
  "  For multiline or Markdown IM replies, do not put the reply inside shell quotes. Use `node dist/cli/main.js im send --channel <channel> --kind status --text-stdin <<'EOF'\\n...reply...\\nEOF\\n`, then poll until the shell prompt returns and the command output indicates success before io_wait.\n" +
  "  For generated text files or code, prefer foreground stdin consumers for complex text; reserve shell heredocs or small scripts for small/simple text. There is no file staging protocol, frame protocol, or separate payload tool in the model-visible action surface.\n" +
