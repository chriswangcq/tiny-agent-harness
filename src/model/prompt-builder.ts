...
const SYSTEM_MESSAGE =
  ...
  "- stash_file: stage complete generated file bytes in harness state. This does not write the workspace. After stash_file returns, use bash to run `node dist/cli/main.js file materialize <stashId> <target-path>` or stream bytes with `node dist/cli/main.js file cat <stashId>`.\n" +
  ...
  "  For user-visible IM replies, use standard shell stdin forms with `--text-stdin`. For anything beyond a simple short phrase, write or materialize `reply.md` first and prefer input redirection: `node dist/cli/main.js im send --channel <channel> --kind status --text-stdin < reply.md`; alternatively stream a stashed reply with `node dist/cli/main.js im send --channel <channel> --kind status --text-stdin < <(node dist/cli/main.js file cat <stashId>)`. File/stash contents are not shell-parsed. Quoted heredoc stdin is allowed only for simple short phrases, for example `node dist/cli/main.js im send --channel <channel> --kind status --text-stdin <<'IM'\\nDone.\\nIM\\n`. Do not put long Markdown, Chinese/emoji-heavy paragraphs, tables, generated reports, or multiline summaries into an IM heredoc. Other standard stdin forms exist, such as `producer | cmd`, `cmd < <(producer)`, and bash/zsh here-string `cmd <<< \"$text\"`; use them only when they make the command simpler. Do not use `im send --text` from the agent.\n" +
  ...
  "To reply, use IM send with --text-stdin through bash. Use quoted heredoc only for simple short phrases; for longer replies, materialize `reply.md` and send it with `< reply.md`, or stream a stashed reply with `file cat` process substitution.\n" +
